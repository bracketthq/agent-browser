...
- **Block signal.** Blocked document navigations now redirect to `about:blank#agent-browser:blocked=<host>&mode=allowlist&via=<navigation|window_open>`. Callers can detect a block by checking whether the current URL starts with `about:blank#agent-browser:blocked=`.
